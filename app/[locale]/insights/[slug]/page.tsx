...
export async function generateStaticParams() {
  const all = await Promise.all([
    getContentList("insights", "es"),
    getContentList("insights", "en"),
    getContentList("insights", "pt"),
  ]);
...
export async function generateMetadata({
  ...
  const entry = await getContentBySlug("insights", locale, slug);
  if (!entry) {
    return {};
  }

  return buildArticleMetadata(entry, `/insights/${slug}`);
}
